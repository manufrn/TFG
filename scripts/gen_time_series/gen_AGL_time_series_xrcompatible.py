...
if __name__ == '__main__':
    ...
    path_SBE37 = data_dir / 'raw' / 'thermistor_chain' / 'AGL_20181116' / 'SBE37'
    ...
    # theoretical depth
    theoretical_depth = np.array([1, 8, 23, 28, 33, 43, 48, 53, 63, 78, 96, 108, 126, 
                           151, 176, 201])

    # temperature array
    temp_ = np.full((len(date_SBE56), 16), np.nan)
    ...
    temp_[:, 0:6] = temp_SBE56[:, 0:6]
    temp_[up_slice] = temp_up_37    
    temp_[:, 7:-1] = temp_SBE56[:, 6:]
    temp_[low_slice] = temp_low_37




    # print('Cada 120 elementos (10 min) de masked temp, hay 16 medidas?:', )
    #       # (np.isnan(~temp[np.argmax(np.in1d(date_SBE56, date_up_37))::120]).sum(.all())
    # print('Cada 60 elementos (5 min) de masked temp, hay 15 medidas o más?:',) 
          # (temp[np.argmax(np.in1d(date_SBE56, date_low_37))::60].count(axis=1) >= 15).all())

    # preassure array
    meassured_depth_ = np.full((len(date_SBE56), 16), np.nan)
    meassured_depth_[:, 0:6] = depth_SBE56[None, :6]
    meassured_depth_[up_slice] = depths_up_37[min_up_37_idx:np.where(date_up_37 == date_SBE56[last_up])[0][0] +1]
    meassured_depth_[:, 7:-1] = depth_SBE56[None, 6:]
    meassured_depth_[low_slice] = depths_low_37[min_low_37_idx:np.where(date_low_37 == date_SBE56[last_low])[0][0] +1]
    ...
    temp_[slice_1] = np.nan
    temp_[slice_2] = np.nan
    meassured_depth_[slice_1] = np.nan
    meassured_depth_[slice_2] = np.nan
    ...
    with netCDF4.Dataset(output_path, mode='w', format='NETCDF4') as ds:
        ...
        temp = ds.createVariable('temp', 'f8', ('date', 'depth',))
        date = ds.createVariable('date', 'i4', ('date', ))
        depth = ds.createVariable('depth', 'i4', ('depth',))
        meassured_depth = ds.createVariable('meassured_depth', 'f4', ('date', 'depth',))
        ...
        meassured_depth[:, :] = meassured_depth_
