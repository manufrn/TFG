...
def wavelet_spectrum(signal, mother, dt, dj):
    n = signal.size
    s0 = 2 * dt
...
def significance(signal, power, mother, dt, scales):

    n = signal.size
    alpha, _, _ = pycwt.ar1(signal)
...
def complete_plot(signal, time, power, period, coi, sig95, levels, dt):
...
def new_complete_plot(signal, time, power, period, coi, sig95, levels, glbl_power, glbl_signif, fft_theor, fftfreqs, fft_power, var, ylim=None):
...
def wavelet_spectrum_and_significance(signal, date, mother=None, period=[None, None]):
    slice_ = slice(*period)
    signal = signal.loc[slice_]
    date = date.loc[slice_]

    if mother is None:
        mother = pycwt.Morlet(6)


    dt = int(date[1] - date[0]) / 1e9
    dj = 0.25

    signal_norm = detrend_and_normalize(signal)

    power, scales, period, coi, fft_power, fft_freqs = wavelet_spectrum(signal_norm, mother,
                                                                        dt, dj)

    sig95, glbl_power, glbl_signif, fft_theor = significance(signal_norm, power, mother, dt,
                                                             scales, var)
